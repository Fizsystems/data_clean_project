...
def populate_travelers_table():
    ...
    for _, row in travelers_df.iterrows():
        traveler_table.insert("", "end", values=(row['traveler_id'], row['name'], row['email'], row['phone'], row['passport_number']))
...
def populate_travels_table(filtered_df=None):
    ...
    for _, row in df.iterrows():
        travel_table.insert("", "end", values=(row['travel_id'], row['traveler_id'], row['destination'], row['departure_date'], row['return_date'], row['purpose']))
...
def on_traveler_select(event):
    ...
    if selected == "All Travelers":
        ...
        traveler_id = travelers_df[travelers_df['name'] == selected]['traveler_id'].values[0]
...
tk.Button(btn_frame, text="Load Travelers CSV", command=lambda: load_travelers(None)).grid(row=0, column=0, padx=10)
tk.Button(btn_frame, text="Load Travels CSV", command=lambda: load_travels(None)).grid(row=0, column=1, padx=10)
...
# Tables
traveler_table = ttk.Treeview(root, columns=("ID","Name","Email","Phone","Passport"), show="headings", height=8)
for col in traveler_table["columns"]:
    traveler_table.heading(col, text=col)
traveler_table.pack(pady=10, fill="x")

travel_table = ttk.Treeview(root, columns=("Travel ID","Traveler ID","Destination","Departure","Return","Purpose"), show="headings", height=10)
...
tk.Button(chart_btn_frame, text="Save Chart as PNG", command=save_chart_png).grid(row=0, column=0, padx=10)
tk.Button(chart_btn_frame, text="Save Chart as PDF", command=save_chart_pdf).grid(row=0, column=1, padx=10)
